...
async function main() {
  ...
  const { wordCount, isSimpleFoundationsFrench } = await import("../src/lib/foundations/level");
  const { foundationsRecall } = await import("../src/lib/foundations/exercise");
  ...
  for (const provider of ["codex", "claude"] as const) {
    ...
    assert.deepEqual({ prompt: exercise.prompt, target: exercise.target }, foundationsRecall(source));
    ...
    for (const entry of [
      { target: "faire une randonnée", prompt: "to go for a hike", topic: "activities", challenge: "standard" as const },
      ...
      assert.deepEqual({ prompt: simple.prompt, target: simple.target }, foundationsRecall(a2));
      console.log(provider, "A2", entry.challenge, JSON.stringify({ prompt: simple.prompt, target: simple.target }));
    }
    const fixed = { prompt: "Translate: some water", target: "de l’eau", rubric: "Recall a word group; no full sentence is required. The partitive de l’eau and the equivalent quantity un peu d’eau are both valid.", provider, fallback: false };
    const bare = await foundationsAI.grade(source, fixed, "de l'eau");
    assert.equal(bare.verdict, "CORRECT");
    const alternate = await foundationsAI.grade(source, fixed, "un peu d'eau");
    assert.equal(alternate.provider, provider);
    assert.equal(alternate.verdict, "CORRECT");
    const mistake = await foundationsAI.grade(source, fixed, "de eau");
