...
export default function ConversationPage() {
  ...
  return <main className="container max-w-2xl py-6">
    ...
    <h1 className="mb-5 text-2xl font-semibold">Conversation practice</h1>
    {!sessionId ? <Card><CardContent className="space-y-4 py-10 text-center"><MessageSquare className="mx-auto h-9 w-9"/><Button onClick={start} disabled={busy}>{busy && <Loader2 className="h-4 w-4 animate-spin"/>}Start in French</Button>{error && <p className="text-sm text-destructive">{error}</p>}</CardContent></Card> : <>
