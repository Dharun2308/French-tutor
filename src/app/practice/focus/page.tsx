...
export default function FocusPage() {
  ...
  if (!session.items.length) return <main className="container max-w-xl py-8"><h1 className="text-2xl font-semibold">{session.caughtUp ? "All caught up" : "Build your focus practice"}</h1><p className="mt-3 text-muted-foreground">{session.caughtUp ? "No phrases are due right now." : "Import lesson notes to build a focus session."}</p><Button asChild className="mt-5"><Link href={session.caughtUp ? "/topics" : "/import"}>{session.caughtUp ? "Practise a topic" : "Import notes"}</Link></Button></main>;
  if (index >= session.items.length) return <main className="container max-w-xl py-8"><Card><CardContent className="py-10 text-center"><h1 className="text-2xl font-semibold">Focus session complete</h1><div className="mt-5 flex flex-wrap justify-center gap-2"><Button onClick={startFresh} disabled={busy}>Start another focus</Button><Button asChild variant="outline"><Link href="/progress">See progress</Link></Button></div>{error && <p className="mt-3 text-destructive">{error}</p>}</CardContent></Card></main>;
  if (!item) return null;
