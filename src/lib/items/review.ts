...
export type ItemReviewInput = z.infer<typeof ItemReviewBody>;
export type ReviewTransaction = Parameters<Parameters<typeof db.transaction>[0]>[0];

/** Keep scheduling, evidence and session progress in the caller's transaction. */
export async function recordItemReview(tx: ReviewTransaction, body: ItemReviewInput, now = new Date()) {
