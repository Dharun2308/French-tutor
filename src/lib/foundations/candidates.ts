...
export async function foundationsCandidates(mix: FoundationsMix = "blend", now = new Date()) {
  ...
  const [personal, ranked, noteReviews, previous, base] = await Promise.all([
    ...
    db.select({ ...getTableColumns(foundationsReviews), version: sql<number>`json_extract(${foundationsSessions.data}, '$.version')` })
  ...
  for (const item of personal) {
    ...
    const face = foundationsCard(item);
    const retry = history[0]?.version === FOUNDATIONS_VERSION && history[0].rating < 2 && (!recent[0] || history[0].ratedAt.getTime() >= recent[0].ratedAt.getTime());
    ...
    sources.push({
      ...
      reason: repeated ? "Extra practice for a familiar expression" : memory.lastRating === 1 ? "You rated this Hard" : memory.lastRating === 0 ? "You rated this Again" : item.reviewCount ? "Your lesson phrase is due" : "A basic expression from your notes",
  ...
  for (const phrase of base) {
    ...
    sources.push({
      ...
      reason: repeated ? "Extra practice for a familiar expression" : memory.lastRating === 1 ? "You rated this Hard" : memory.lastRating === 0 ? "You rated this Again" : reviewed ? "An everyday expression ready for review" : "Everyday French basics",
      ...
      retryExercise: history[0]?.version === FOUNDATIONS_VERSION && history[0].rating < 2 && (!phrase.lastReviewedAt || history[0].ratedAt.getTime() >= phrase.lastReviewedAt.getTime()) ? history[0].exercise : undefined,
    });
  }
  return {
    ...
    history: previous.filter(r => r.version === FOUNDATIONS_VERSION).slice(0, 60).map(r => ({ prompt: r.exercise.prompt, target: r.exercise.target })),
  };
}
